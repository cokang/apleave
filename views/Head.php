		...
		function check_leave_availability(duration,year,probation){
			...
			if($("#leave_type").val()!="0"){
				...
				$.get("<?php echo base_url ('index.php/check_availability') ?>?year="+year+"&type="+duration,"",function(data){
					...
					// alert("("+json['annual_leave']+"!=undefined ? "+json['entitled']+" : 0) + ("+json['carry_fwd_leave']+"!=undefined ? "+json['carry_fwd_leave']+" : 0) - "+json['ALtaken']+" - "+json['ELtaken']+" - "+json['FSEtaken']+" - "+json['MLEtaken']+" - "+json['PLEtaken']+" - "+json['MRLEtaken']+" - "+json['ULEtaken']+" - "+json['STLEtaken']+" - "+json['TLEtaken']+" - "+json['HLEtaken']+" - ("+json['SLEtaken']+"!=undefined ? "+json['SLEtaken']+" : 0)");
					if (annualB < 0){
						var ALbalance = 0;
					}
					else{
