...
class AP_leave {
	...
	public function get_leave_detail($leaveacc, $tleavetaken, $hajj='', $year, $leave_type=''){
		// $this->CI->load->model('display_model''',TRUE);
		$is_selected_leave = (count($leave_type)==1) ? true : false;
		$selected_leave = ($is_selected_leave==true) ? $leave_type[0]->id : 0;
		// echo $leave_type[0]->id;die;
      ...
      $data['hajjstat']=0;
			...
			if( $is_selected_leave ){
				...
				if( $selected_leave==1 || $selected_leave==3 ){
					...
					if(  $selected_leave==3 ){//echo $row->ELtaken;die;
					...
					// echo "<pre>";var_export($leave_type[11]);die;
					...
					// echo "<pre>";var_export($leave_type[12]);die;
    ...
    $holiday_array = array();
		...
		$holidayArr = $this->get_holiday_state( $year );
